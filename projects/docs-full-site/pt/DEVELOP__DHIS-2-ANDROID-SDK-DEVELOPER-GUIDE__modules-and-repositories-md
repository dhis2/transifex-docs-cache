...
Os módulos estão na camada abaixo de `D2`. Eles actuam como um invólucro para funcionalidades relacionadas. Um módulo inclui alguns repositórios relacionados e pode expor alguns serviços e auxiliares.
...
## Lidando com tipos de retorno: RxJava
...
- **Para facilitar o tratamento assíncrono dos objectos retornados.** A maioria das acções no SDK são demoradas e devem ser executadas em um thread secundário. Esses tipos de retorno forçam o aplicativo a lidar com esse comportamento assíncrono.
...
Repositórios oferecem uma sintaxe de construtor com validação em tempo de compilação para acessar os recursos. Uma consulta típica é composta de alguns modificadores (filtro, pedido, campos aninhados) e termina com uma ação (get, count, getPaged, ...).
...
retornará um objeto aninhado `TrackedEntityType`.
