...
2. [Importer](#generic-metadata) le fichier de métadonnées dans le DHIS2.
...
### Métadonnées génériques { #generic-metadata }

Le package contient certaines métadonnées qui sont généralement communes ou partagées par de nombreux programmes de santé. Ces métadonnées ont un préfixe "GEN" pour signifier qu'elles sont génériques et réutilisables dans d'autres parties de la configuration DHIS2. Il est probable que vous ayez déjà des éléments similaires dans votre base de données. Ces éléments génériques sont utilisés dans certains indicateurs et sont inclus dans le fichier de métadonnées agrégées de surveillance/SIMR.

Éléments de données génériques utilisés dans le package SC MEV

| Objet | UID | Point d'extrémité de l'API |
| --- | --- | --- |
| GEN - Population | DkmMEcubiPv | `../api/dataElements.json?filter=name:like:GEN - Population` |
| GEN - Population <  15 ans | cPLAnOTldta | `../api/dataElements.json?filter=name:like:GEN - Population` |
| GEN - Naissances vivantes de la population | iKxybad85rv | `../api/dataElements.json?filter=name:like:GEN - Population` |
| GEN - Population par semaine | iLEkjJcYTJd | `../api/dataElements.json?filter=name:like:GEN - Population` |

Options de catégories, catégories, combinaisons de catégories pour les âges

| Objet | UID | Point d'extrémité de l'API |
| --- | --- | --- |
| 0-4 ans | UPvKbcqTEY3 | `../api/categoryOptions.json?filter=name:like:years` |
| Plus de 5 ans | afpMrUgPzl3 | `../api/categoryOptions.json?filter=name:like:years` |
| Âge (surveillance) | wAZr8Iv7S64 | `../api/categories.json?filter=name:like:Age` |
| Âge (surveillance) | eg8enaFY861 | `../api/categoryCombos.json?filter=name:like:Age` |

### Prédicteurs utilisant le niveau d’unité d’organisation District { #predictors-using-organisation-unit-level-district }

Le package contient de nombreux prédicteurs qui utilisent une balise pour le niveau d’unité d’organisation District (NUO). Le point d'extrémité d’API pour ces prédicteurs correspondra à : `../api/predictors?filter=organisationUnitLevels:!null`.

La balise utilise l’étiquette <OU_LEVEL_DISTRICT_UID>. Avant de tenter d’importer le package, vous devez remplacer cette étiquette par l’UID du NUO équivalent dans votre système.
