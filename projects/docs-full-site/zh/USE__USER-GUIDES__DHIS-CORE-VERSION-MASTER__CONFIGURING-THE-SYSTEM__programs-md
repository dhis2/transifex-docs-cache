---
edit_url: "https://github.com/dhis2/dhis2-docs/blob/master/src/user/configure-programs-in-the-maintenance-app.md"
...
| 节目类型 | 描述 | 使用示例 |
|---|---|---|
| 事件项目 | 单一活动 *无* 注册程序（匿名程序或 SEWoR）<br>   <br> 通过卫生系统跟踪匿名的个人事件。没有任何个人或实体参与这些单独的交易。 <br>   <br> 只有一个节目阶段。 | 无需在系统中注册任何信息即可记录健康案例。 <br> <br> 记录调查数据或监测线路列表。 |
| 单级跟踪程序 | *带有*注册程序 (SEWR) 的单一活动<br>   <br> 通过卫生系统的每笔单独交易来跟踪实体（人、商品等）<br>   <br> 只有一个节目阶段。 <br>   <br> 跟踪实体实例 (TEI) 只能在计划中注册一次。 | 记录出生证明和死亡证明。 |
| 多阶段跟踪程序 | *带*注册程序 (MEWR) 的多活动<br>   <br> 通过卫生系统的每笔单独交易来跟踪实体（人、商品等）<br>   <br> 有多个程序阶段。 | 母亲健康计划分为 ANC 访视（2-4+）、分娩、PNC 访视等阶段。 |
...
| 追踪实体类型 | 可通过系统跟踪的实体类型。可以是从人到商品的任何东西，例如药品或人。 <br>   <br> 一个程序必须有一个被跟踪的实体。要将被跟踪实体实例注册到程序中，实体的被跟踪实体和程序的被跟踪实体必须相同。 <br>        <br>     **注意** <br>       <br>     必须仅使用一个跟踪实体来指定程序。只有与计划的跟踪实体相同的跟踪实体才能注册到该计划。 | 创建、编辑、克隆、共享、删除、显示详细信息和翻译 |
| 跟踪实体属性 | 用于注册被跟踪实体的额外信息。 <br>   <br> 可以在程序之间共享。 | 创建、编辑、克隆、共享、删除、显示详细信息和翻译 |
...
3.  输入程序详细信息，然后单击下一步。
    ...
    | **已完成的活动截止日期** |定义您可以编辑已完成事件的天数。这意味着当事件完成并且指定的到期天数已过时，事件将被锁定。 <br>       <br> 如果您将“已完成的事件到期天数”设置为“10”，则事件将在完成日期后十天被锁定。在此日期之后，您将无法再编辑该事件。
    | **有效期类型** <br>       <br> **到期日** |到期天数定义了上一期间结束后的多少天可以编辑事件。期间类型由到期期间类型定义。这意味着，当自上一期间的结束日期起经过指定的到期天数时，该期间的事件将被锁定。 <br>       <br> 如果您将到期类型设置为“每月”，并将到期天数设置为“10”，并且月份为 10 月，则您无法在 11 月 10 日之后添加或编辑到 10 月的事件。 |
    | **完成后阻止报名表** |选择复选框可在该程序的事件完成后阻止输入表单。 <br>       <br> 这意味着在将状态重置为不完整之前，无法更改报名表中的数据。 |
    ...
    | **验证策略** |设置服务器和客户端验证要求。 <br>  <br> 无论验证策略如何，始终执行数据类型验证。例如，整数字段永远不会存储包含文本的字段。 <br> - **完成时** 此选项将强制在完成事件时修复必填字段和错误消息，但事件可以在不通过这些验证要求的情况下保存到服务器。由于遗留原因，这始终是跟踪器程序的验证策略，其中事件中的每个数据值在输入数据时都存储到服务器。 <br> - **更新和插入时** 无论完成状态如何，此选项将在将事件保存到服务器时强制执行必需的字段验证。使用此选项时，如果没有通过验证，则无法存储任何事件。 |
    | **预生成事件UID** |选择复选框以预先生成唯一的事件 ID 号。 |
    | **报告日期说明** |输入报告日期的描述。 <br>       <br> 此描述显示在案例条目表中。 |
...
3.  （可选）为每个数据元素添加其他设置：
    ...
    | **桌面渲染类型** |警告：尚未实施。 <br>       <br> 可用于为桌面（即 Web 界面）选择不同的渲染类型。可用选项根据数据元素的值类型而有所不同。例如，对于数值，您可以选择“默认”、“值”、“滑块”、“线性比例”和“微调器”。 |
...
8.  在 **收件人** 字段中，选择谁应该接收该计划
    ...
    |网络挂钩 | Web 挂钩是发送到 Web 挂钩 URL 字段中配置的外部 URL 的自动 HTTP 消息。通知模板变量将作为 HTTP 请求中的键值对发送。 | - |
...
| 程序规则组件 | 描述 |
|---|---|
| 聚集类型 | 聚合类型决定了程序指标的聚合方式。可以使用以下聚合类型：<br>  * 平均值<br>  * 平均值（数字）<br>  * 平均值（数字，分解）<br>  * 平均值（组织单位层次结构中的总和）<br>  * 平均值（数字总和）<br>  * 平均值（数字总和，分解）<br>  * 平均值（是/否）<br>  * 计数<br>  * 自定义<br>  “自定义”聚合类型允许您在表达式中内嵌指定聚合类型。所有其他聚合类型都应用于整个表达式。 <br>  使用“自定义”聚合类型可能会导致上述计算顺序的例外，其中可以计算和聚合表达式的各个部分，而不是在聚合之前计算整个表达式。 <br>  * 默认 <br>  * 最大<br>  * 最小值 <br>  * 无 <br>  * 标准差<br>  * 总和<br>  * 方差 |
| 分析类型 | 可用的分析类型有*事件*和*注册*。 <br>   <br> 分析类型定义计划指标是基于事件还是计划注册来计算。这会影响可以进行的计算类型。 <br>  * 事件意味着每个事件作为独立行存在的数据源。这适用于执行聚合，例如计数和求和。 <br>  * 注册意味着单个注册的所有事件都组合在同一行上的数据源。这允许进行可以比较来自计划注册内的各个计划阶段的事件数据的计算。 |
| 组织单位字段 | 确定将哪个组织单位分配给计划指标值。 <br>  <br>  对于活动计划（无需注册），选项为：<br>  * 活动组织单位（默认）：活动发生地<br>  * 分配给计划的值类型组织单位（如果有）的任何数据元素 <br>  <br>  对于跟踪程序（需要注册）和分析类型*注册*，选项为：<br>  * 注册组织单位：创建跟踪实体实例的地方 <br>  * 注册组织单位（默认）：跟踪的实体实例在此计划中注册的位置 <br>  * 起始组织单位的所有者：报告期开始时所跟踪实体实例的所有者 <br>  * 最终组织单位的所有者：报告期结束时所跟踪实体实例的所有者 <br>  <br>  对于跟踪程序（需要注册）和分析类型*事件*，选项为：<br>  * 活动组织单位（默认）：活动发生地<br>   * 分配给计划的值类型组织单位（如果有）的任何数据元素 <br>  * 注册组织单位：创建跟踪实体实例的地方 <br>  * 注册组织单位：跟踪的实体实例在此计划中注册的位置 <br>  * 起始组织单位的所有者：报告期开始时所跟踪实体实例的所有者 <br>  * 最终组织单位的所有者：在报告期结束时所跟踪实体实例的所有者 |
| 分析周期边界 | 定义程序指标计算的边界。边界决定哪些事件或注册包含在聚合中，始终与聚合报告期的开始和结束相关。创建程序指标时，将根据分析类型预先选择默认边界。 <br>  * 对于分析类型*事件*，默认边界将配置为封装在报告期开始之后和报告期结束之前具有事件日期的任何事件。 <br>  * 对于分析类型*注册*，默认边界将封装所有注册，注册日期为报告日期开始之后和报告期结束之前。此外，默认注册计划指示器会评估所有计划阶段的最新事件，无论日期如何。 <br>   <br> 可以更改上限和下限以包括相对于报告周期更长或更短的周期，或者删除其中一个界限 - 实际上返回特定周期之前或之后的所有数据。还可以添加更多约束，例如使登记计划指示符仅包括截至给定时间点的事件数据。 <br>  * 边界目标：可以是*事件日期*、*事件日期*、*注册日期*或*自定义*。指定受边界约束的内容。 <br>   <br>  *自定义* 用于创建以日期数据元素、跟踪实体属性或程序阶段中事件的存在为目标的边界。这是通过表单上的自定义表达式完成的：<br>  - 日期类型的数据元素：#{programStageUid.dataElementUid}。 <br>  `#{A03MvHHogjR.a3kGcGDCuk6}` <br>  - 跟踪日期类型的实体属性：#{attributeUid}。 <br>  `A{GPkGfbmArby}` <br>  - 特定程序阶段中存在一个事件：PS_EVENTDATE:programStageUid。 <br>  `PS_EVENTDATE:A03MvHHogjR` <br>  **注意** 此边界目标仅适用于 Analytics 类型注册 <br>  * 分析周期边界类型：定义边界是结束边界 - 从“之前...”开始，还是开始边界 - “之后...”。还定义边界是与汇总报告期结束还是与汇总报告期开始相关。 <br>  * 按金额偏移周期：在某些情况下，例如同期群分析，在运行数据透视表和报告时，边界应相对于聚合报告周期进行偏移。按量计算的偏移周期用于将当前边界及时向后（负）或向前（正）移动。金额和期间类型共同决定偏移量的大小。一个例子是，在为 1 年队列制定简单的入学队列计划指标时，用“-1”和“年”来抵消计划指标的每个边界可能就足够了 <br>  * 期间类型：见上文。可以是任何时期，例如*每周*或*每季度*。 |
| 表达 | 该表达式定义了指标的计算方式。表达式可以包含对各种实体的引用，在计算指标时这些实体将替换为相关值：<br>  * 数据元素：将替换为进行计算的时间段和组织单位的数据元素值。指的是程序阶段和数据元素。 <br>  * 属性：将替换为进行计算的人员/跟踪实体的属性值。 <br>  * 变量：将替换为与计划相关的特殊值，包括事件日期和人员注册日期、当前日期以及计算所针对的时间段和组织单位的表达式中的值计数。 <br>  * 常量：将被常量的值替换。 <br>   <br> 该表达式是一个数学表达式，也可以包含运算符。 <br>   <br> 对于分析类型*事件*的单事件程序和跟踪器程序，将评估*每个事件*表达式，然后根据其聚合类型进行聚合。 <br>   <br> 对于分析类型*注册*的跟踪程序，将评估*每个注册*的表达式，然后根据其聚合类型进行聚合。 |
| 过滤 | 该过滤器应用于事件并过滤用于指标计算的数据源。 IE。在评估指标表达式之前，过滤器会应用于事件集。过滤器的计算结果必须为 true 或 false。它过滤器应用于每个单独的事件。如果过滤器计算结果为 true，则该事件稍后将包含在表达式计算中，否则将被忽略。过滤器可以以与表达式类似的方式包含对数据元素、属性和常量的引用。 <br>   <br> 程序指示符过滤器还可以使用逻辑运算符。这些运算符可用于形成最终计算结果为 true 或 false 的逻辑表达式。例如，您可以断言多个数据元素必须是特定值，或者特定属性的数值必须小于或大于常量。 |
...
| 值类型 | 示例语法 |
|---|---|
| 整数<br>   <br> 负整数<br>   <br> 正整数或零整数 <br>   <br> 正整数<br>   <br> 编号<br>   <br> 百分比 | 数字字段，可作为表达式或过滤器用于聚合：<br>  `#{mCXR7u4kNBW.K0A4BauXJDl} >= 3` |
...
| d2：countIfValue | （数据元素，值） | 仅对注册计划指标有用。计算注册过程中与给定程序阶段和数据元素的给定文字值匹配的数据值的数量。参数数据元素由 #{programStage.dataElement} 语法提供。该值可以是硬编码文本或数字，例如“No_anemia”（如果仅应计算包含此文本的值）。 |
| d2:countIf条件 | （数据元素，条件） | 仅对注册计划指标有用。计算注册过程中与给定程序阶段和数据元素的给定条件标准相匹配的数据值的数量。参数数据元素由 #{programStage.dataElement} 语法提供。条件以单引号中的表达式形式提供，例如，如果只计算小于 10 的值，则为 '<10'。 |
| 如果 | （布尔表达式、真表达式、假表达式） | 计算布尔表达式，如果 true 返回 true 表达式值，如果 false 返回 false 表达式值。这与 d2:condition 函数相同，只是 boolean-expr 没有被引用。 |
| 是 | (表达式 [, 表达式 ...] 中的 expr1) | 如果 expr1 等于以下任意表达式，则返回 true，否则返回 false。 |
| 一片空白 | （目的） | 如果对象值缺失 (null)，则返回 true，否则返回 false。 |
...
| / | 两个数相除 |
| % | 两个数的模 |
| && | 逻辑与。仅当左侧和右侧的表达式为真时才为真。左侧和右侧可以是是/否、仅是或括号中的子表达式。 |
| &#124;&#124; | 逻辑或。当左侧表达式或右侧表达式为真时为真。左侧和右侧可以是是/否、仅是或括号中的子表达式。 |
| > | 左边的数大于右边的数 |
| >= | 左边的数大于或等于右边的数 |
| < | 左边的数小于右边的数 |
| <= | 左边的数小于或等于右边的数。 |
...
| 功能 | 论据 | 描述 |
|---|---|---|
| d2：ceil | （数字） | 将输入参数**向上舍入**到最接近的整数。 <br> 示例：<br>  `d2:ceil(#{hemoglobinValue})` |
| d2：楼层 | （数字） | 将输入参数**向下**舍入到最接近的整数。 <br> 生成妇女怀孕周数的示例。请注意，子表达式 #{gestationalAgeDays}/7 在执行下限函数之前进行评估：<br>  `d2:floor(#{gestationalAgeDays}/7)` |
| d2：圆 | （数字[，小数]） | 将输入参数舍入为最接近的整数。可以提供可选的第二个参数来指定数字要舍入的小数位数。 <br> 示例： d2:round(1.25, 1) = 1.3 |
| d2：模数 | （数字，数字） | 将第一个参数除以第二个参数时生成模数。 <br> 生成女性当前怀孕周的天数的示例：<br>  `d2:modulus(#{gestationalAgeDays},7)` |
| d2：zing | （数字） | 如果值是负数，则将数字类型的参数评估为零，否则为值本身。 |
| d2：oizp | （数字） | 如果值为零或正数，则将类型为number的参数评估为1，否则为零。 |
| d2：串联 | （对象，[，对象，对象，...]） | 从输入参数生成一个字符串连接字符串。支持任意数量的参数。主要用于未来的操作类型，例如使用 d2:concatenate('weeks','+','gestationalageDays') 显示孕龄。 |
| d2：daysBetween | （日期，日期） | 生成第一个参数和第二个参数之间的天数。当第一个参数日期早于第二个参数日期时，数字将为正数 - 在相反的情况下，数字将为负数。静态日期格式为“yyyy-MM-dd”。 <br> 例如，根据末次月经和当前事件日期计算女性的孕龄（以天为单位）：<br>  `d2:daysBetween(#{lastMenstrualDate},V{event_date})` |
| d2：星期之间 | （日期，日期） | 生成第一个参数和第二个参数之间的整周数。当第一个参数日期早于第二个参数日期时，数字将为正数 - 在相反的情况下，数字将为负数。静态日期格式为“yyyy-MM-dd”。 |
| d2：monthsBetween | （日期，日期） | 生成第一个参数和第二个参数之间的完整月份数。当第一个参数日期早于第二个参数日期时，数字将为正数 - 在相反的情况下，数字将为负数。静态日期格式为“yyyy-MM-dd”。 |
| d2：年之间 | （日期，日期） | 生成第一个参数和第二个参数之间的年数。当第一个参数日期早于第二个参数日期时，数字将为正数 - 在相反的情况下，数字将为负数。静态日期格式为“yyyy-MM-dd”。 |
| d2：addDays | （日期、编号） | 基于第一个参数日期生成日期，添加第二个参数天数。 <br> 根据末次月经计算预产期的示例：<br>  `d2:addDays(#{lastMenstrualDate},'283')` |
| d2：计数 | （源字段） | 计算为参数中的源字段输入的值的数量。源字段参数是程序中定义的源字段之一的名称 - 请参阅示例 <br> 示例用法，其中 #{previousPregnancyOutcome} 是可重复程序阶段“先前怀孕”中的源字段之一：<br>  `d2:count('之前的妊娠结果')` |
| d2：countIfValue | （源字段、文本） | 计算为第一个参数中的源字段输入的匹配值的数量。仅计算与第二个参数匹配的出现次数。源字段参数是程序中定义的源字段之一的名称 - 请参阅示例。 <br> 示例用法，其中 #{previousPregnancyOutcome} 是可重复程序阶段“先前怀孕”中的源字段之一。以下函数将生成之前以堕胎结束的怀孕次数：<br>  `d2:countIfValue('先前怀孕结果','堕胎')` |
| d2：countIfZeroPos | （源字段） | 计算为参数中的源字段输入的零或正值的数量。源字段参数是程序中定义的源字段之一的名称 - 请参阅示例。 <br> 示例用法，其中 #{fundalHeightDiscrepancy} 是程序中的源字段之一，可以是正值，也可以是负值。以下函数将生成正数出现的次数：<br>  `d2:countIfZeroPos('宫底高度差异')` |
| d2：hasValue | （源字段） | 如果参数源字段包含值，则计算结果为 true；如果未输入值，则计算结果为 false。 <br> 用法示例，查找源字段是否 #{currentPregnancyOutcome} 尚未填写：<br>  `d2:hasValue('当前妊娠结果')` |
| d2：zpvc | （对象，[，对象，对象，...]） | 返回给定对象参数中数字零和正值的数量。可以提供任意数量的参数。 |
| d2：validatePattern | （文本、正则表达式模式） | 如果输入文本与提供的正则表达式模式完全匹配，则评估为 true。正则表达式需要转义。 <br> 示例表达式，如果数字不在模式 9999/99/9 上则触发操作, <br>  `!d2:validatePattern(A{nrc},'\\d{6}\/\\d{2}\/\\d')` <br>示例表达式，如果地址不包含字母或空格，则触发操作，然后是空格，然后是数字：  <br>`!d2:validatePattern(A{registrationAddress},'[\\w ]+ \\d+')` <br> 示例，如果名称包含任何数字则触发操作：<br>  `!d2:validatePattern(A{name},'[^\\d]*')` <br> 示例表达式，如果手机号码包含非法号码序列 555:  <br>`d2:validatePattern(A{mobile} ,'.*555.*')` |
...
|  17   | EXP_DATE                  | 到期日期（年月日）                                                                                                                   | 8            |
|  20   | 变体                   | 内部产品变体                                                                                                                  | 4            |
|  21   | SERIAL_NUMBER             | 序列号                                                                                                                             | 变量     |
|  22   | CPV                       | 消费产品变体                                                                                                                   | 变量     |
|  235  | TPX                       | 第三方控制的全球贸易项目编号 (GTIN) 序列化扩展 (TPX)                                                      | 变量     |
|  240  | ADDITIONAL_ID             | 制造商分配的附加产品标识                                                                             | 变量     |
|  241  | CUSTOMER_PART_NUMBER      | 客户零件编号                                                                                                                       | 变量     |
|  242  | MTO_VARIANT_NUMBER        | 定制版本号                                                                                                             | 变量     |
|  243  | 聚氯乙烯                       | 包装件编号                                                                                                                 | 变量     |
|  250  | SECONDARY_SERIAL          | 二级序列号                                                                                                                   | 变量     |
|  251  | REF_TO_SOURCE             | 对源实体的引用                                                                                                                | 变量     |
|  253  | GDTI                      | 全局文档类型标识符                                                                                                           | 变量     |
|  254  | GLN_EXTENSION_COMPONENT   | GLN 扩展组件                                                                                                                    | 变量     |
|  255  | GCN                       | 全球优惠券号 (GCN)                                                                                                                 | 变量     |
|  30   | VAR_COUNT                 | 变量计数                                                                                                                            | 变量     |
|  310* | NET_WEIGHT_KG             | 净重，公斤（可变衡量贸易项目                                                                                         | 变量     |
|  311* | LENGTH_M                  | 长度或第一尺寸，米（可变测量贸易项目）                                                                            | 变量     |
|  312* | WIDTH_M                   | 宽度、直径或第二尺寸，米（可变测量贸易项目）                                                                 | 变量     |
|  313* | HEIGHT_M                  | 深度、厚度、高度或三维度，米（可变测量贸易项目）                                                        | 变量     |
|  314* | 面积_M2                   | 面积，平方米（可变衡量贸易项目）                                                                                         | 变量     |
|  315* | NET_VOLUME_L              | 净体积，升（可变计量贸易项目）                                                                                           | 变量     |
|  316* | NET_VOLUME_M3             | 净体积，立方米（可变计量贸易项目）                                                                                     | 变量     |
|  320* | NET_WEIGHT_LB             | 净重，磅（可变衡量贸易项目）                                                                                           | 变量     |
|  321* | 长度_I                  | 长度或第一尺寸，英寸（可变测量贸易项目）                                                                            | 变量     |
|  322* | 长度_F                  | 长度或第一尺寸，英尺（可变测量贸易项目）                                                                              | 变量     |
|  323* | LENGTH_Y                  | 长度或第一尺寸、码（可变测量贸易项目）                                                                             | 变量     |
|  324* | 宽度_I                   | 宽度、直径或第二尺寸，英寸（可变测量贸易项目）                                                                 | 变量     |
|  325* | 宽度_F                   | 宽度、直径或第二尺寸、英尺（可变测量贸易项目）                                                                   | 变量     |
|  326* | 宽度_Y                   | 宽度、直径或第二尺寸、码（可变测量贸易项目）                                                                   | 变量     |
|  327* | 高度_I                  | 深度、厚度、高度或三维尺寸，英寸（可变测量贸易项目）                                                         | 变量     |
|  328* | 高度_F                  | 深度、厚度、高度或三维度、英尺（可变测量贸易项目）                                                           | 变量     |
|  329* | 高度_Y                  | 深度、厚度、高度或三维、码（可变测量贸易项目）                                                          | 变量     |
|  330* | GROSS_WEIGHT_GF           | 物流重量，公斤                                                                                                                 | 变量     |
|  331* | LENGTH_M_LOG              | 长度或第一尺寸，米                                                                                                          | 变量     |
|  332* | 宽度_M_LOG               | 宽度、直径或第二尺寸（米）                                                                                               | 变量     |
|  333* | HEIGHT_M_LOG              | 深度、厚度、高度或三维（米）                                                                                       | 变量     |
|  334* | AREA_M2_LOG               | 面积，平方米                                                                                                                       | 变量     |
|  335* | 体积_L_LOG              | 物流体积，升                                                                                                                    | 变量     |
|  336* | 体积_M3_日志             | 物流体积，立方米                                                                                                              | 变量     |
|  337* | KG_PER_M2                 | 公斤每平方米                                                                                                                | 变量     |
|  340* | 毛重_WHEIGHT_LB          | 物流重量，磅                                                                                                                    | 变量     |
|  341* | LENGTH_I_LOG              | 长度或第一尺寸，英寸                                                                                                          | 变量     |
|  342* | LENGTH_F_LOG              | 长度或第一尺寸，英尺                                                                                                            | 变量     |
|  343* | LENGTH_Y_LOG              | 长度或第一尺寸，码                                                                                                           | 变量     |
|  344* | 宽度_I_LOG               | 宽度、直径或第二尺寸（英寸）                                                                                               | 变量     |
|  345* | 宽度_F_LOG               | 宽度、直径或第二尺寸，英尺                                                                                                 | 变量     |
|  346* | 宽度_Y_LOG               | 宽度、直径或第二维、码                                                                                               | 变量     |
|  347* | 高度_I_LOG              | 深度、厚度、高度或三维（英寸）                                                                                       | 变量     |
|  348* | HEIGHT_F_LOG              | 深度、厚度、高度或三维、英尺                                                                                         | 变量     |
|  349* | 高度_Y_LOG              | 深度、厚度、高度或三维、码                                                                                        | 变量     |
|  350* | 区域_I2                   | 面积，平方英寸（可变测量贸易项目）                                                                                          | 变量     |
|  351* | 区域_F2                   | 面积，平方英尺（可变衡量贸易项目）                                                                                            | 变量     |
|  352* | 区域_Y2                   | 面积，平方码（可变计量贸易项目）                                                                                          | 变量     |
|  353* | AREA_I2_LOG               | 面积，平方英寸                                                                                                                        | 变量     |
|  354* | AREA_F2_LOG               | 面积，平方英尺                                                                                                                          | 变量     |
|  355* | AREA_Y2_LOG               | 面积、平方码                                                                                                                         | 变量     |
|  356* | NET_WEIGHT_T              | 净重，金衡盎司（可变计量贸易项目）                                                                                      | 变量     |
|  357* | NET_VOLUME_OZ             | 净重（或体积），盎司（可变计量贸易项目）                                                                               | 变量     |
|  360* | NET_VOLUME_Q              | 净体积，夸脱（可变计量贸易项目）                                                                                           | 变量     |
|  361* | NET_VOLUME_G              | 净体积，加仑美国（可变衡量贸易项目）                                                                                     | 变量     |
|  362* | 音量_Q_日志              | 物流体积，夸脱                                                                                                                    | 变量     |
|  363* | 体积_G_LOG              | 物流量，加仑（美国）                                                                                                              | 变量     |
|  364* | 音量_I3                 | 净体积，立方英寸（可变衡量贸易项目）                                                                                     | 变量     |
|  365* | 音量_F3                 | 净体积，立方英尺（可变衡量贸易项目）                                                                                       | 变量     |
|  366* | 体积_Y3                 | 净体积，立方码（可变计量贸易项目）                                                                                      | 变量     |
|  367* | 体积_I3_日志             | 物流体积，立方英寸                                                                                                               | 变量     |
|  368* | 音量_F3_日志             | 物流体积，立方英尺                                                                                                                 | 变量     |
|  369* | VOLUME_Y3_LOG             | 物流体积，立方码                                                                                                                | 变量     |
|  37   | 数数                     | 物流单元中包含的贸易项目或贸易项目件数                                                                     | 变量     |
|  390* | 数量                    | 适用应付金额或优惠券面额，当地货币                                                                                  | 变量     |
|  391* | AMOUNT_ISO                | 使用 ISO 货币代码支付的适用金额                                                                                           | 变量     |
|  392* | 价格                     | 适用的应付金额，单一货币区（可变计量贸易项目）                                                              | 变量     |
|  393* | 价格_ISO                 | 采用 ISO 货币代码的适用应付金额（可变计量贸易项目）                                                             | 变量     |
|  394* | PRCNT_关闭                 | 优惠券折扣百分比                                                                                                            | 变量     |
|  395* | PRICE_UOM                 | 每个计量单位单一货币区的应付金额（可变计量贸易项目）                                                      | N4+N6        | 变量     |
|  400  | ORDER_NUMBER              | 客户采购订单号                                                                                                            | 变量     |
|  401  | 吉尼卡                      | 全球托运识别码 (GINC)                                                                                        | 变量     |
|  403  | 路线                     | 路由代码                                                                                                                               | 变量     |
|  410  | SHIP_TO_GLOB_LOC          | 运送至/交付至全球地点编号 (GLN)                                                                                          | 变量     |
|  411  | BILL_TO_LOC               | 帐单收件人/发票收件人全球地点编号 (GLN)                                                                                          | 变量     |
|  412  | PURCHASED_FROM            | 从全球位置号码 (GLN) 购买                                                                                                | 变量     |
|  413  | SHIP_FOR_LOG              | 运送/交付 - 转发至全球位置号码 (GLN)                                                                           | 变量     |
|  414  | LOC_NUMBER                | 物理位置的标识 - 全球位置号 (GLN)                                                                       | 变量     |
|  415  | PAY_TO                    | 发票方的全球位置编号 (GLN)                                                                                        | 变量     |
|  416  | PROD_SERV_LOC             | 生产或服务地点的全球位置编号 (GLN)                                                                         | 变量     |
|  417  | 派对                     | 当事人全球位置号码 (GLN)                                                                                                         | 变量     |
|  420  | SHIP_TO_POST              | 运送至/递送至单一邮政机构内的邮政编码                                                                          | 变量     |
|  421  | SHIP_TO_POST_ISO          | 运送至/递送至带有 ISO 国家/地区代码的邮政编码                                                                                     | 变量     |
|  422  | 起源                    | 贸易项目的原产国                                                                                                          | 变量     |
|  423  | COUNTRY_INITIAL_PROCESS   | 初始加工国家                                                                                                              | 变量     |
|  424  | COUNTRY_PROCESS           | 加工国家                                                                                                                      | 变量     |
|  425  | COUNTRY_DISASSSEMBLY       | 拆卸国家/地区                                                                                                                     | 变量     |
|  426  | COUNTRY_FULL_PROCESS      | 国家覆盖全流程链                                                                                                        | 变量     |
|  427  | ORIGIN_SUBDIVISION        | 原产国细分                                                                                                              | 变量     |
|  4300 | SHIP_TO_COMP              | 收货/送货公司名称                                                                                                          | 变量     |
|  4301 | SHIP_TO_NAME              | 收货地址/送货地址联系方式                                                                                                               | 变量     |
|  4302 | SHIP_TO_ADD1              | 收货地址/送货地址第 1 行                                                                                                        | 变量     |
|  4303 | SHIP_TO_ADD2              | 收货地址/送货地址第 2 行                                                                                                        | 变量     |
|  4304 | SHIP_TO_SUB               | 送货到/送货到郊区                                                                                                                 | 变量     |
|  4305 | SHIP_TO_LOCALITY          | 收货/送货地点                                                                                                               | 变量     |
|  4306 | SHIP_TO_REG               | 收货/送货地区                                                                                                                 | 变量     |
|  4307 | SHIP_TO_COUNTRY           | 收货/送货国家/地区代码                                                                                                          | 变量     |
|  4308 | SHIP_TO_PHONE             | 收货地址/送货地址电话号码                                                                                                      | 变量     |
...
|  4315 | RTN_TO_LOCALITY           | 返回当地                                                                                                                         | 变量     |
|  4316 | RTN_TO_REG                | 返回地区                                                                                                                           | 变量     |
|  4317 | RTN_TO_COUNTRY            | 返回国家/地区代码                                                                                                                     | 变量     |
|  4318 | RTN_TO_POST               | 返回邮政编码                                                                                                                      | 变量     |
|  4319 | RTN_TO_PHONE              | 返回电话号码                                                                                                                 | 变量     |
|  4320 | SRV_DESCRIPTION           | 服务代码说明                                                                                                                   | 变量     |
|  4321 | DANGEROUS_GOODS           | 危险品标志                                                                                                                      | 变量     |
|  4322 | AUTH_LEAV                 | 有权离开                                                                                                                         | 变量     |
|  4323 | SIG_REQUIRED              | 需要签名标志                                                                                                                    | 变量     |
|  4324 | NBEF_DEL_DT               | 不早于交货日期时间                                                                                                              | 变量     |
|  4325 | NAFT_DEL_DT               | 不晚于交货日期时间                                                                                                               | 变量     |
|  4326 | REL_DATE                  | 发布日期                                                                                                                               | 变量     |
|  7001 | 诺森                       | 北约库存编号 (NSN)                                                                                                                   | 变量     |
|  7002 | MEAT_CUT                  | UN/ECE 肉类屠体和分割分类                                                                                              | 变量     |
|  7003 | EXP_TIME                  | 到期日期和时间                                                                                                                   | 变量     |
|  7004 | ACTIVE_POTENCY            | 活性效力                                                                                                                             | 变量     |
|  7005 | CATCH_AREA                | 捕捞面积                                                                                                                                 | 变量     |
|  7006 | FIRST_FREEZE_DATE         | 首次冻结日期                                                                                                                          | 变量     |
|  7007 | HARVEST_DATE              | 收获日期                                                                                                                               | 变量     |
|  7008 | AQUATIC_SPECIES           | 渔业品种                                                                                                               | 变量     |
|  7009 | FISHING_GEAR_TYPE         | 渔具类型                                                                                                                          | 变量     |
|  7010 | PROD_METHID               | 制作方法                                                                                                                          | 变量     |
|  7020 | REFURB_LOT                | 翻新批次 ID                                                                                                                       | 变量     |
|  7021 | FUNC_STAT                 | 功能状态                                                                                                                          | 变量     |
|  7022 | REV_STAT                  | 修订状态                                                                                                                            | 变量     |
|  7023 | GIAI_ASSEMBLY             | 程序集的全局单个资产标识符 (GIAI)                                                                                   | 变量     |
|  703* | PROCESSOR_NUMBER          | 具有 ISO 国家代码的处理器数量                                                                                                  | 变量     |
|  7040 | UIC_EXT                   | 带有扩展 1 和进口商索引的 GS1 UIC                                                                                                | 变量     |
|  710  | NHRN_PZN                  | 国家医疗报销号 (NHRN) - 德国 PZN                                                                              | 变量     |
|  711  | NHRN_CIP                  | 国家医疗报销号 (NHRN) - 法国 CIP                                                                               | 变量     |
|  712  | NHRN_CN                   | 国家医疗报销号 (NHRN) - 西班牙 CN                                                                                 | 变量     |
|  713  | NHRN_DRN                  | 国家医疗报销号 (NHRN) - 巴西 DRN                                                                               | 变量     |
|  714  | NHRN_AIM                  | 国家医疗报销号 (NHRN) - 葡萄牙 AIM                                                                             | 变量     |
|  723* | CERT_NUMBER               | 认证参考                                                                                                                    | 变量     |
|  7240 | 协议                  | 协议号                                                                                                                                | 变量     |
|  8001 | 方面                | 卷材产品（宽度、长度、芯径、方向、接头）                                                                           | 变量     |
|  8002 | CMT_NUMBER                | 蜂窝移动电话标识符                                                                                                       | 变量     |
|  8003 | 格莱伊                      | 全球可返还资产标识符 (GRAI)                                                                                                  | 变量     |
|  8004 | GIAI                      | 全球个人资产识别码（GIAI）                                                                                                  | 变量     |
|  8005 | PRICE_PER_UNIT            | 每计量单位的价格                                                                                                                  | 变量     |
|  8006 | 信息技术计划                      | 单个贸易项目的识别（ITIP）                                                                                    | 变量     |
|  8007 | 国际银行账号                      | 国际银行帐号 (IBAN)                                                                                                   | 变量     |
|  8008 | PROD_TIME                 | 生产日期和时间                                                                                                                | 变量     |
|  8009 | 奥普森                    | 光学可读传感器指示器                                                                                                        | 变量     |
|  8010 | 消费者物价指数                      | 组件/零件标识符 (CPID)                                                                                                           | 变量     |
|  8011 | CPID_SERIAL               | 组件/部件标识符序列号 (CPID SERIAL)                                                                                      | 变量     |
|  8012 | 版本                   | 软件版本                                                                                                                           | 变量     |
|  8013 | GMN                       | 全球型号 (GMN)                                                                                                                  | 变量     |
|  8017 | GSRN_PROVIDER             | 全球服务关系号 (GSRN)，用于标识提供服务的组织与服务提供商之间的关系  | 变量     |
|  8018 | GSRN_RECIPIENT            | 全球服务关系号 (GSRN)，用于标识提供服务的组织与服务接受者之间的关系 | 变量     |
|  8019 | SRIN                      | 服务关系实例号 (SRIN)                                                                                                    | 变量     |
|  8020 | REF_NUMBER                | 付款单参考编号                                                                                                              | 变量     |
|  8026 | ITIP_内容              | 识别物流单元中包含的贸易项目 (ITIP)                                                               | 变量     |
|  8110 | COUPON_USA                | 用于北美的优惠券代码识别                                                                                        | 变量     |
|  8111 | 积分                    | 优惠券的忠诚度积分                                                                                                                 | 变量     |
|  8121 | POSITIVE_OFFER_COUPON_USA | 无纸化优惠券代码识别，适用于北美                                                                              | 变量     |
|  8200 | 产品_URL               | 扩展打包 URL                                                                                                                     | 变量     |
|  90   | 协议_内部        | 贸易伙伴之间共同商定的信息                                                                                       | 变量     |
...
| 变量 | 类型 | 描述 |
|---|---|---|
| V {current_date} | （日期） | 包含执行规则时的当前日期。 <br> 表达式示例：<br>  `d2:daysBetween(#{symptomDate},V{current_date}) < 0 `|
| V {event_date} | （日期） | 包含当前事件执行的事件日期。规则作为注册表格的一部分执行时将没有值。 |
| V {event_status} | （细绳） | 包含当前活动或注册的状态。 <br> 检查状态的示例表达式为：<br>  `V{event_status} =='完成'` |
...
| V{enrollment_id} | （细绳） | 当前注册的通用标识符字符串(UID)。对于单项活动计划没有价值。 |
| V {event_id} | （细绳） | 当前事件上下文的通用标识符字符串（UID）。当规则作为注册表的一部分执行时，不会有值。 |
| V {orgunit_code} | （细绳） | 包含链接到当前注册的组织单位的代码。对于单个事件程序，将使用当前事件组织单元中的代码。 <br> 检查组织单位代码是否以 WB_ 开头的示例表达式：<br>  `d2:left(V{orgunit_code},3) == 'WB_'` |
| V {environment} | （细绳） | 包含代表规则的当前运行时环境的代码。可能的值为“ WebClient”，“ AndroidClient”和“ Server”。当程序规则仅应在一种或多种客户端类型中运行时可以使用。 |
| V {program_stage_id} | （细绳） | 包含触发规则的当前程序阶段的ID。这可用于在特定程序阶段运行规则，或避免在某些阶段执行。在TEI注册表单中执行规则时，变量将为空。 |
| V {program_stage_name} | （细绳） | 包含触发规则的当前程序阶段的名称。这可用于在特定程序阶段运行规则，或避免在某些阶段执行。在TEI注册表单中执行规则时，变量将为空。 |
| V{completed_date} | （细绳） | 此变量包含触发此规则的事件的完成日期。如果事件尚未完成，则"completed_date"不包含任何内容。 |
...
有关配置以及“来自约束”和“至约束”含义的更多信息，请参阅[关系模型](#relationship_model_relationship_type)。
...
9.  选择“来自约束”。这限制了关系中可以包含的实体类型。 [关系模型](#relationship_model_relationship_type)。选择“来自约束”后，您可以选择应在“跟踪器捕获”和“来自约束”的捕获中的关系小部件中显示哪些属性或数据元素。该列表将根据约束条件而有所不同：
    * 当选择“跟踪实体实例”时，仅选择跟踪实体类型，在配置的跟踪实体类型属性之间进行选择
    * 选择“跟踪实体实例”，然后选择跟踪实体类型和程序时，在为跟踪实体类型和程序配置的属性之间进行选择
    * 选择“注册计划”时，请在已为计划配置的属性之间进行选择
    * 选择“节目或节目阶段中的事件”时，请在已为该事件节目或节目阶段配置的数据元素之间进行选择

10. 选择“至约束”。这限制了关系中可以包含的实体类型。 [关系模型](#relationship_model_relationship_type)。重复选择应在“至约束”的关系小部件中显示的属性或数据元素。
...
7. （可选）在 **字段掩码** 字段中，您可以键入用于提供
   属性正确格式的提示。 **注意：到目前为止仅在 DHIS2 Android Capture 应用程序中实现，而不是在 Capture 和 Tracker Capture Web 应用程序中实现。**
...
9.  在 **值类型** 字段中，选择要使用的数据类型
    ...
    |---|---|
    |年龄 | - |
    |坐标 |指定为以十进制度数表示的经度和纬度的点坐标。所有坐标均应以“-19.23 , 56.42”格式指定，并用逗号分隔经度和纬度。 |
    |日期 |日期在数据输入中呈现为日历小部件。 |
    |日期和时间 | - |
    |电子邮件 | - |
    |文件 |您可以在其中存储外部文件（例如文档和照片）的文件资源。 |
    |图片|与文件类似，但仅限于图像。 |
    |整数|任何整数（正数和负数），包括零。 |
    |信| - |
    |长文|文本价值。呈现为表单中的文本区域。 |
    |负整数 |任何小于（但不包括）零的整数。 |
    |数量 |任何带有单个小数点的实数值。不支持千位分隔符和科学计数法。 |
    |百分比 | 0 到 100 之间的整数。
    |电话号码 ||
    |正整数|任何大于（但不包括）零的整数。 |
    |零整数的正数 |任何正整数，包括零。 |
    |组织单位| - |
    |单位间隔|任何大于或等于 0 且小于或等于 1 的实数。
    |文字|文本价值。每个值允许的最大字符数为 50,000。 |
    |时间 |时间以 HH:mm 格式存储。 <br>       <br> HH 是 0 到 23 之间的数字<br>       <br> mm 是 00 到 59 之间的数字 |
    |追踪助理| - |
    |用户名 |呈现为带有用户列表和搜索字段的对话框。用户需要“查看用户”权限才能使用此数据类型 |
    |是/否 |布尔值，在数据输入中呈现为下拉列表。 |
    |仅是 |真实值，在数据输入中呈现为复选框。 |
...
9.  Select an **Aggregation type**.
    ...
    |聚合运算符|描述 |
    |---|---|
    |平均 |对期间维度和组织单位维度中的值进行平均。 |
    |平均值（组织单位层次结构中的总和）|期间维度中数据值的平均值，组织单位维度中数据值的总和。 |
    |计数 |数据值的计数。 |
    |最小 |数据值的最小值。 |
    |最大|数据值的最大值。 |
    |无 |任何维度都不进行聚合。 |
