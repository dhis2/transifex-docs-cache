...
| 国家卫生管理者 <br> <br> 省卫生管理者 | 监控和分析数据 | 访问报告模块、**地图**、**数据质量**应用程序和仪表板。 | 无需访问即可输入数据、修改数据元素或数据集。 |
| 国家卫生信息系统部门官员 (HISO) <br> <br> 地区健康记录和信息官员 (DHRIO) <br> <br> 设施健康记录和信息官员 (HRIO) | 输入来自无法直接输入的设施的数据 <br> <br> 监控、评估和分析数据 | 访问所有分析和验证应用程序 <br> <br> 访问**数据输入**应用程序。 | -- |
| 数据录入员 | -- | -- | -- |
...
DHIS 2 部署的事实上的标准方式已成为_在线_，这意味着应用程序的单个实例设置在连接到 Internet 的服务器上，并且所有用户使用网络浏览器通过 Internet 连接到应用程序。这要归功于互联网可用性（主要是移动互联网）的稳步增长、现成且廉价的云计算资源的提供以及 DHIS 2 不需要大量带宽的事实。这些发展使得使用移动互联网调制解调器（也称为加密狗）访问在线服务器成为可能，即使是在大多数农村地区。
...
**软件应用**：中央服务器部署方式意味着应用可以集中升级和维护。当发布具有新功能和错误修复的应用程序的新版本时，可以将其部署到单个在线服务器。下次最终用户通过 Internet 连接时，所有更改都将反映在客户端。这显然对改进系统的过程产生了巨大的积极影响，因为新功能可以立即分发给用户，所有用户都将访问相同的应用程序版本，并且可以即时整理和部署错误和问题 <!-- Bugs can be deployed on-the-fly! --> .
...
因此，如果互联网连接受限且数据输入表单很小，则有两种解决连接问题的可能性：Android 数据捕获应用程序和 Web 数据输入离线功能。
...
DHIS 2 应用程序的数据捕获允许用户使用 Android 设备将数据输入到 DHIS 2 服务器。该应用程序下载从服务器输入数据所需的表单实例，并将它们存储在设备上。这意味着用户可以离线输入他们分配到的设施的数据，然后在有网络覆盖时将其上传到 DHIS 2 服务器。
...
安装并启动应用程序后，将要求用户提供其国家 DHIS 2 的 URL、用户名和密码，然后点击登录。
...
- 使用 Android 捕获应用程序在本地输入数据，如果用户在那里参加定期会议，则在可以访问互联网的上层同步数据。这只有在表格很小的情况下才可行
...
本地化涉及使应用程序适应特定位置。在特定国家/地区实施 DHIS 2 时，应分配足够的资源来翻译和本地化应用程序（如果需要）。必须考虑用户界面元素、消息、布局、日期和时间格式、货币和其他方面的翻译。除了翻译用户界面本身之外，还必须考虑翻译数据库中包含的元数据内容。
...
DHIS 2 通过使用 Java 属性字符串和 PO 文件支持用户界面的国际化 (i18n)。当消息来自后端 Java 服务器时使用 Java 属性文件，而 PO 文件用于用 JavaScript 编写的前端应用程序。 DHIS 2 Android 应用程序使用特定的 XML 格式。
...
> **重要**
>
> 在 DHIS 2 的各个部分中，有许多源字符串，例如 "dd MMM yyyy 'to '" 用于日期/时间格式。部分值不应翻译，因为它实际上是使用的特殊格式字段通过 Java 或 JavaScript 插入或格式化字符串。在此示例中，**可以**翻译的值部分将是“to”，例如西班牙语中的“a”。应该**不**翻译的特殊字符串是“dd MMM yyyy”。如果翻译这些日期格式模板字符串，可能会导致应用程序出错！

> **重要**
>
> 一些特殊变量（例如 {0} ）使用大括号。这表示将由应用程序替换为数字或其他值的变量。您必须将此变量表示法放在正确的位置，并确保不要修改它。
...
> **提示**
>
> 确保有特定应用程序的完整翻译，例如“Tracker Capture”，您需要确保翻译完成：
>
...
即应用程序只有一个源资源（`en.pot`），但我们添加了从 2.31（v31）到最新开发（主）的版本。版本显示在“类别”字段中，并且作为资源名称的前缀也可见，例如`v31--en-pot`。
...
除了用户界面的翻译，DHIS 2 还支持数据库中元数据内容的本地化。可以通过**维护应用程序**翻译单个对象，但为了更好地支持标准翻译工作流程，为此目的开发了一个专门的应用程序。

可以在 **Maintenance app > Locales** 中添加新的元数据语言环境。

### DHIS 2 Translations应用程序 { #translations-app }

DHIS 2 **翻译应用程序**可用于将所有元数据（数据元素、类别、组织单位等）翻译成数据库中存在的任何语言环境。
